// import MovieCard from "@/components/MovieCard"; // Removed unused import to fix module not found error

async function fetchMovies(page: number = 1) {
  const url = `https://api.themoviedb.org/3/discover/movie?page=${page}`;

  const res = await fetch(
    `https://api.themoviedb.org/3/movie/popular?api_key=${process.env.TMDB_API_KEY}`, // v3 ключ из .env.local
    {
      headers: {

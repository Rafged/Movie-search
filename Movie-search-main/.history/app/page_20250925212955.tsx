...
// helper: ensure debounced functions can be cancelled if they have .cancel
function _maybeCancel(handler){ if(!handler) return; if(typeof handler.cancel === "function") handler.cancel(); }
    ...
    <Card key={m.id} hoverable style={{ width: 220 }}>
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ background: getVoteColor(m.userRating ?? vote_average || 0), color:'#000', padding:'4px 8px', borderRadius:6 }}>
            {(m.userRating ?? vote_average||0).toFixed(1)}
          </div>
          <Rate allowHalf value={(ratedMovies[m.id] && ratedMovies[m.id].rating) ? ratedMovies[m.id].rating/2 : 0} count={8} onChange={(v)=>handleRate(m.id, v*2)} />
        </div>
      </div>
    </Card>
  );
          ...
          {loading ? <div style={{textAlign:'center', padding:40}}><Spin size="large" /></div> : (
            <>
              ...
              <div style={{ textAlign:'center', marginTop:12 }}>
                <Pagination current={page} total={total_results} pageSize={10} onChange={(p)=>{ setPage(p); doSearch(q,p); }} />
              </div>
            </>
          )}
        </Tabs.TabPane>

...
export default function MovieList({ movies = [], onRate }) {
          ...
          <div style={{ display:'flex', gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ marginTop:8, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div style={{ background: getVoteColor(m.userRating ?? vote_average||0), padding:'4px 8px', borderRadius:6 }}>{(m.userRating ?? vote_average||0).toFixed(1)}</div>
  ...
  {movie.genres
    ...
    : movie.genre_ids?.map((id) => (
        <span key={id} className="genre-chip">{genreMap[id] || "Other"}</span>
      ))}
</div>
        </Card>
      ))}
